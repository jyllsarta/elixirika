...
module.exports = class Character4_Default {
  onGameStart(character, model) {
    const { enemies } = character.getCallback(
  ...
  onSendToStarPalette = (character, model, field) => {
    ...
    if (currentMp < maxMp) {
      character.uniqueParameters.mp = Math.min(
        maxMp,
        currentMp + field.score(),
      );
    }
  ...
  onFillDraw(character, model, sentCardLength) {
    ...
    let nextEnemy = character.uniqueParameters.enemies.find(
      (enemy) => enemy.hp > 0,
    );
    if (!nextEnemy) {
  ...
  canGetSenderCardFromSkill(character, model) {
    return (
      character.uniqueParameters.mp >= Constants.costOfAbilityAddSenderCard
    );
  }
};
