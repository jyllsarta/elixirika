...
module.exports = class Controller {
  ...
  loadOperationHistory(historyString) {
    ...
    this.model = new Model(
      operations.characterId,
      operations.chapterId,
      operations.seed,
    );
  ...
  fillDraw(force = false, muted = false) {
    if (
      !force &&
      this.model.hand.field.cards.length >= Constants.handCardNumber
    ) {
      return;
    }
    ...
    this.model.character.getCallback("onFillDraw", this.model.chapter.index)(
  ...
  _doSendCardToBoard(card, fromField, toField) {
    if (
      !this.model.cardStackRule(this.model.character, this.model, card, toField)
    ) {
    ...
    this.model.character.getCallback("onSendCard", this.model.chapter.index)(
      this.model.character,
      this.model,
      card,
      toField,
    );
    const cleared = this.model.character.getCallback(
      "isClearedMainTarget",
      this.model.chapter.index,
    )(this.model.character, this.model);
    if (cleared) {
  ...
  _commitSenderCard(toField) {
    ...
    this.model.character.getCallback(
      "onSendToStarPalette",
      this.model.chapter.index,
    )(this.model.character, this.model, newField);
    this.model.messageManager.register("sendToStarPalette", {
      toField: newField,
    });
  }
  ...
  unstageStagedCard() {
    ...
    stagedField.field.sendCardById(card.id, this.model.hand.field, {
      index: handIndex,
    });
  ...
  sendHandToEmptyPocketAbility(handIndex) {
    const emptyPocket = this.model.character.uniqueParameters.abilities?.find(
      (ability) =>
        ability.category === "cardPocket" &&
        ability.card === null &&
        ability.count > 0,
    );
    if (!emptyPocket) {
      this.model.messageManager.register(
        "cannotIgniteAbilityCardPocketMaxCard",
      );
      this.model.soundManager.register("miss");
  ...
  describeSupportAbility(args) {
    const { index } = args;
    const category =
      this.model.character.uniqueParameters.abilities[index]?.category || "";
    const abilityName =
      "describeAbility" +
      category.slice(0, 1).toUpperCase() +
      category.slice(1, Math.inf);
    // これはホバーごとに発火しまくるので、メッセージが更新済みの場合はアップデートをしない
    if (
      [abilityName].indexOf(this.model.messageManager.currentMessage?.when) !==
      -1
    ) {
      return;
    }
  ...
  sendPlayLog() {
    ...
    return axios
      .post("/square/register_log", {
        _csrf_token: document.querySelector("meta[name=csrf-token]").attributes[
          "content"
        ].textContent,
  ...
  prepareSendToAbility() {
    ...
    if (
      [
        "abilityCardPocketSendPrepare",
        "abilityCardPocketSendPrepareNoAbility",
      ].indexOf(this.model.messageManager.currentMessage?.when) !== -1
    ) {
      return;
    }
    // 送れないのに送ろうとしたら専用メッセージ
    if (
      !this.model.character.uniqueParameters.abilities?.find(
        (ability) => ability.category === "cardPocket",
      )
    ) {
      this.model.messageManager.register(
        "abilityCardPocketSendPrepareNoAbility",
