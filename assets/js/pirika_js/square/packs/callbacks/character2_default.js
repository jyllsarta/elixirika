...
module.exports = class Character2_Default {
  ...
  onSendToStarPalette(character, model, field) {
    const { banSendCard } = character.getCallback(
  ...
  onSendCard(character, model, card, field) {
    const { banCardGap } = character.getCallback(
      "starPaletteParameter",
      model.chapter.index,
    )();
    if (banCardGap) {
  ...
  onFillDraw(character, model, sentCardLength) {
    const { banDiscard } = character.getCallback(
      "starPaletteParameter",
      model.chapter.index,
    )();
    if (banDiscard) {
  ...
  canGetSenderCardFromSkill(character, model) {
    return character.uniqueParameters.abilities?.some(
      (ability) => ability.card?.category == "sender",
    );
  }
};
