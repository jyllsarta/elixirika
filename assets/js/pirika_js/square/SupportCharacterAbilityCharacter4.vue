  ...
  <div class="area">
    ...
    <div class="mp">
      ...
      ><span>
        <NumeratableNumber
          class="mpvalue"
          :number="character.uniqueParameters.mp"
          :speed="0.1"
          :scaled="true"
        ></NumeratableNumber> </span
      ><span>{{ maxMpMessage }}</span>
    </div>
    <div class="bar">
      <div class="bg"></div>
    ...
    <transition-group class="buttons" name="ability" tag="span">
      <SupportCharacterAbilityButton
        ...
        @popclick="
          $emit('guiEvent', { type: 'igniteSupportAbility', index: index })
        "
        @popmouseover="
          $emit('guiEvent', { type: 'describeSupportAbility', index: index })
        "
        :key="ability.slot"
        :index="index"
        :ability="ability"
...
export default {
  ...
  computed: {
    mpBarWidth() {
      const percentage =
        Math.min(this.character.uniqueParameters.mp / 100, 1) * 100;
      return `${percentage}%`;
    },
    maxMpMessage() {
      if (
