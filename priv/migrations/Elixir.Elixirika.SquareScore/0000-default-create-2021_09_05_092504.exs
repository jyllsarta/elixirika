defmodule Elixirika.SquareScore.Migration0000 do
  ...
  def __migration__(:structure) do
    %Yacto.Migration.Structure{field_sources: %{character_id: :character_id, id: :id, inserted_at: :inserted_at, playlog: :playlog, score: :score, updated_at: :updated_at, user_id: :user_id}, fields: [:id, :score, :character_id, :user_id, :playlog, :inserted_at, :updated_at], meta: %{attrs: %{character_id: %{default: 0, null: false}, score: %{default: 0, null: false}, user_id: %{null: false}}, indices: %{}}, source: "elixirika_squarescore", types: %{character_id: :integer, id: :id, inserted_at: :naive_datetime, playlog: :text, score: :integer, updated_at: :naive_datetime, user_id: :integer}}
  end
