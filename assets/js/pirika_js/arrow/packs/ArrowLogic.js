...
class ArrowLogic {
  constructor() {
    ...
    this.onlineRanking = new OnlineRanking(
      location.href,
      location.href + "/ranking",
      location.href + "/high_score",
    );
    this.highScore = 0;
    this.loadSounds();
    this.reset();
  ...
  proceedTimerAndFireEvent(timeDelta) {
    ...
    if (
      this.isThisFrameTimerReached(timeDelta, this.charge, Constants.chargeMax)
    ) {
    ...
    if (Constants.healIntervalTimeSeconds < this.healEventTimer) {
      ...
      const energyRecovered =
        Constants.addEnergyAmountPerEvent * (this.hpRate() / 2 + 0.5);
      this.energy += energyRecovered;
      // いま energy最大になった場合 音を鳴らす
      if (
        this.isThisFrameTimerReached(
          energyRecovered,
          this.energy,
          Constants.energyMax,
        )
  ...
  sweepRemovedBalls() {
    this.balls = this.balls.filter(
      (ball) => ball.removedAt === null || this.timeScore < ball.removedAt + 1,
    );
  }
  ...
  checkDamage(timeDelta) {
    ...
    for (let ball of this.balls) {
      ...
      if (
        distance <
        Constants.shaveDamageRadius *
          (this.hpRate() * Constants.ratioOfHpRateToHitBox +
            Constants.minimumHitBoxSizeRate)
      ) {
    ...
    if (this.hp <= 0) {
      ...
      this.onlineRanking.submit(
        this.username,
        this.score(),
        this.removeScore,
        this.timeScore,
        (results) => {
          this.onSucceedSendResult(results);
        },
      );
      this.gameState = 2;
    }
  ...
  createRandomBall() {
    const vx =
      Math.random() * Constants.maxBallVelocityX -
      Constants.maxBallVelocityX / 2;
    const vy =
      Math.random() * Constants.maxBallVelocityY -
      Constants.maxBallVelocityY / 2;
    ...
    const colorId = Math.floor(
      ((velocity * 2) / velocityMax) * Constants.colorIdNum,
    );
  ...
  fetchRanking() {
    this.onlineRanking.getRanking((results) => {
  ...
  fetchHighScore() {
    this.onlineRanking.getHighScore(this.username, (results) => {
      this.onReceiveHighScore(results);
    });
  }
  ...
  loadSounds() {
    ...
    this.soundManager.register(
      "discharge_available",
      "/game/arrow/sounds/discharge_available.wav",
    );
    this.soundManager.register(
      "charge_start",
      "/game/arrow/sounds/charge_start.wav",
    );
    this.soundManager.register(
      "charge_complete",
      "/game/arrow/sounds/charge_complete.wav",
    );
    this.soundManager.register(
      "high_score",
      "/game/arrow/sounds/high_score.wav",
    );
  }
}
